...
$totquery = "SELECT * FROM `ER` WHERE `Ck` = 1 ORDER BY `T` DESC ";
...
$g = 0;
$totquery2 = "SELECT * FROM `ER` WHERE `Ck` = 0 ORDER BY `T` DESC ";
...
<body>
        ...
        <h2 style="text-align:center;margin-top:0px;"><?php echo $top;?></h2>
        <h2 style="font-size:19;">အဌမတန်း</h2>
        <div id="l"><a href="ViewResult.php"><image style = "border-radius:50%;"src = "lg.jpg" alt = "logo" width = "68px" height= "68px"></a></div>
    <table align="center" border=".5" width= "100%" cellspacing="0" cellpadding="5" texalign="center">
    <col width = "50">
    <col width = "200">
    <col width = "50">
    <col width = "50">
    ...
    <tr style="font-size:18;"><th>စဥ်</th><th>အမည်</th></th><th>မြန်မာ</th><th>အင်္ဂလိပ်</th><th>သင်္ချာ</th><th>ပထ၀ီ</th><th>သမိုင်း</th><th>သိပ္ပံ</th><th>စုစုပေါင်း</th><th>အဆင့်</th></tr>
    <?php 
        $nroll = '';
        $nroll = mysqli_num_rows(mysqli_query($con,"SELECT * FROM GE"));
        $arraysize = $nroll;
        $q = "SELECT * FROM `ER` WHERE `Ck` = 1 ORDER BY `T` DESC ";
        ...
        $g = $g+1;
        $s = 0;
        $q2 = "SELECT * FROM `ER` WHERE `Ck` = 0 ORDER BY `T` DESC ";
